...
class InterSlabRDF:
    ...
    def _single_frame(self, g1, g2):
        ...
        pos_1_ndx = np.where(
            (g1.positions[:, 0] > self.cutoff_slab[0])
            & (g1.positions[:, 0] <= self.cutoff_slab[1])
        )[0]
        pos_2_ndx = np.where(
            (g2.positions[:, 0] > self.cutoff_slab[0])
            & (g2.positions[:, 0] <= self.cutoff_slab[1])
        )[0]
        ...
        pairs, dist = distances.capped_distance(
            pos_1, pos_2, self.rdf_settings["range"][1], box=self.init_box
        )
